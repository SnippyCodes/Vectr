            ...
            <div className="w-full max-w-md fade-in">
                ...
                <div className="glass-card p-8">
                    ...
                    <button
                        ...
                        disabled={loading}
                        className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-lg border border-border-default bg-bg-secondary text-text-primary hover:border-accent-cyan transition-all mb-6 disabled:opacity-50 cursor-pointer"
                    ...
                    <form onSubmit={handleEmailAuth} className="space-y-4">
                        <div>
                            ...
                            <input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)}
                                className="input-dark !rounded-lg" placeholder="you@example.com" required autoComplete="email" />
                        </div>
                        <div>
                            <label htmlFor="password" className="text-text-secondary text-sm mb-1 block">Password</label>
                            <input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)}
                                className="input-dark !rounded-lg" placeholder="••••••••" required autoComplete={isSignup ? 'new-password' : 'current-password'} />
                        </div>
                        ...
                        {isSignup && (
                            <div>
                                ...
                                <select id="level" value={level} onChange={e => setLevel(e.target.value)}
                                    className="input-dark !rounded-lg appearance-none cursor-pointer">
                        ...
                        <button type="submit" disabled={loading} className="btn-primary w-full !py-3 text-sm disabled:opacity-50" id="auth-submit-btn">
